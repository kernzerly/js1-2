...
console.log(id);
//id's should only be used for js not styling not best practice
//newer methods:
let tags = document.querySelector('body');
console.log(tags);
let myClass = document.querySelector('.my-header');
console.log(myClass);  //class alone would turn purple
let myId =
...
//try it
// function queryDom(methodName, element){
//   swith(methodName.toLowerCase()){
//     case 'getElemtnsByTagName':
...
myClass.addEventListener('click',
  e=> { alert("hi")})//callback functions  2nd  listening for events
// day 6 try it
...
button.addEventListener('click',e=> {  //call back function
  alert('you hit this button');
})
